...
const DATA={
  businesses:[
    {id:'b1',name:'Borgo Test',comune:'Scilla',provincia:'Reggio Calabria',category:'Ristoranti',status:'pending',email:'borgo@example.test',phone:'',whatsapp:'',website:'',ownerId:'u1',description:'Descrizione attività completa per il test amministrativo CalabriaVera.',verified:false,views:12,phoneClicks:2,whatsappClicks:1,websiteClicks:3,favoriteCount:4,requestCount:2,slug:'borgo-test',updatedAt:'2026-09-10T10:00:00Z'},
    {id:'b2',name:'Mare Test',comune:'Tropea',provincia:'Vibo Valentia',category:'B&B',status:'approved',email:'mare@example.test',phone:'+39000000000',whatsapp:'+39000000000',website:'https://example.test',ownerId:'u2',description:'Seconda descrizione sufficientemente lunga per i controlli SEO amministrativi del portale e per la scheda pubblica.',verified:true,views:20,phoneClicks:5,whatsappClicks:2,websiteClicks:7,favoriteCount:8,requestCount:3,slug:'mare-test',updatedAt:'2026-09-11T11:00:00Z'}
  ],
  users:[
    ...
    {id:'u2',displayName:'Lucia Bianchi',email:'lucia@example.test',role:'USER',status:'active',marketingConsent:false}
  ],
  publicProfiles:[{id:'u1',displayName:'Mario Pubblico',avatarDataUrl:''},{id:'u2',displayName:'Lucia Pubblica',avatarDataUrl:''}],
...
const authStub=`const user={uid:'admin-1',email:'admin@example.test',getIdTokenResult:async()=>({claims:{admin:true}}),getIdToken:async()=> 'test-token'};export async function getCurrentUser(){return user}export async function currentUser(){return user}export function watchAuth(cb){let live=true;queueMicrotask(()=>{if(live)cb(user)});return()=>{live=false}}export async function requireUser(){return user}export {user as __testUser};`;
...
const remoteFirestore=`const D=${json};const rows=n=>(D[n]||[]).map(x=>({...x}));const nameOf=r=>r?.name||String(r?.path||'').split('/')[0]||'';export const collection=(_db,...p)=>({kind:'collection',path:p.join('/'),name:p[0]||''});export const doc=(a,...p)=>p.length?({kind:'doc',path:p.join('/'),name:p[0]||a?.name||'',id:p[p.length-1]}):({kind:'doc',path:(a?.path||'')+'/new',name:a?.name||'',id:'new'});export const where=(field,op,value)=>({type:'where',field,op,value});export const orderBy=(field,dir='asc')=>({type:'orderBy',field,dir});export const limit=n=>({type:'limit',n});export const query=(ref,...constraints)=>({...ref,constraints});export async function getDocs(ref){let a=rows(nameOf(ref));for(const c of ref?.constraints||[]){if(c.type==='where'&&c.op==='==')a=a.filter(x=>x[c.field]===c.value);if(c.type==='orderBy')a=a.sort((x,y)=>String(x[c.field]??'').localeCompare(String(y[c.field]??''))*(c.dir==='desc'?-1:1));if(c.type==='limit')a=a.slice(0,c.n)}const docs=a.map(x=>({id:x.id,ref:{name:nameOf(ref),id:x.id,path:nameOf(ref)+'/'+x.id},data:()=>({...x})}));return{docs,empty:!docs.length,size:docs.length}}export async function getDoc(ref){const x=rows(nameOf(ref)).find(v=>v.id===ref?.id);return{exists:()=>!!x,data:()=>x?({...x}):undefined,id:ref?.id,ref}}export async function setDoc(){}export async function updateDoc(){}export async function deleteDoc(){}export async function addDoc(){return{id:'new'}}export const serverTimestamp=()=> '2026-09-12T12:00:00Z';export const deleteField=()=>null;export const increment=n=>n;export const arrayUnion=(...v)=>v;export const arrayRemove=(...v)=>v;export const writeBatch=()=>({delete(){},set(){},update(){},commit:async()=>{}});export const onSnapshot=(r,cb)=>{queueMicrotask(async()=>cb(await getDocs(r)));return()=>{}};`;
...
async function installMocks(context){
  ...
  for(const f of ['maintenance.js','city-counts.js','full-i18n.js','runtime-idle.js','redesign-interactions.js','privacy-consent.js','i18n-loader.js']) await context.route(`**/assets/js/${f}*`,r=>js(r,f==='maintenance.js'?"document.documentElement.classList.remove('cv-maintenance-pending');export{}":"export{}"));
...
async function settle(p,name){
  await p.locator('#admin-root .admin-layout').waitFor({state:'visible',timeout:15000});
  await p.waitForFunction(()=>{const t=document.querySelector('#admin-root')?.textContent||'';return !/Caricamento (?:area amministrativa aggiornata|amministrazione|centro di controllo|Attività & CRM|comunicazioni|messaggi)/i.test(t)},null,{timeout:15000}).catch(()=>{});
  if(name==='messaggi'){
    await p.locator('#admin-content[data-critical-workbench="ready"]').waitFor({state:'attached',timeout:15000});
    await p.locator('[data-admin-delete-conversation]').waitFor({state:'attached',timeout:15000});
  }else if(name==='attivita'){
    await p.locator('#admin-content [data-quality-extra]').first().waitFor({state:'attached',timeout:15000});
  }else if(name==='utenti'){
    await p.locator('#admin-content [data-admin-site-profile]').first().waitFor({state:'attached',timeout:15000});
  }
  await p.waitForTimeout(350);
}
async function open(browser,origin,path,name){
...
for(const [name,path] of routes){
 const L=await open(browser,'http://127.0.0.1:4190',path,name),R=await open(browser,'http://127.0.0.1:4191',path,name);
 ...
 const bad=[];if(clean(a)!==clean(b))bad.push('admin-root DOM mismatch');if(clean(at)!==clean(bt))bad.push('admin-root text mismatch');if(menu.join('|')!=='Centro controllo|Attività & CRM|Recensioni|Segnalazioni|Utenti|Comunicazioni|Messaggi|Blog|Categorie & Comuni')bad.push('final menu mismatch');if(L.errors.length)bad.push('legacy errors: '+L.errors.join('; '));if(R.errors.length)bad.push('react errors: '+R.errors.join('; '));console.log(`${name}: ${bad.length?bad.join(' | '):'OK'}`);if(bad.length)failures.push({name,bad,legacy:clean(a).slice(0,2200),react:clean(b).slice(0,2200)});await L.c.close();await R.c.close();
}
