...
const [oracleRoot, reactRoot, outputRoot = '/tmp/cv-visual/out-v4'] = process.argv.slice(2);
if (!oracleRoot || !reactRoot) throw new Error('usage: react-final-visual-v4.mjs <oracle-dist> <react-dist> [output]');
...
try {
  ...
  if (failures.length) {
    ...
    console.log('Visual parity v4: tutte le route deterministic/stable UI <= 1% pixel diff');
  }
} finally {
  await browser.close();
